...
def get_rays(H, W, focal, pose):
    ...
    # dirs = 
    dirs = torch.stack([(u - W * 0.5) / focal, -(v - H * 0.5) / focal, -torch.ones_like(u)], dim=-1)
    dirs = dirs.to(pose.device)
    ...
    # rays_d = 
    rays_d = torch.matmul(pose[:3, :3].float(), dirs[..., None].float()).squeeze(-1)
...
def positional_encoder(p, L_embed=6):
    ...
    for i in range(L_embed):
        ...
        rets.append(torch.sin(scale * p))
        rets.append(torch.cos(scale * p))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
def render(model, rays_o, rays_d, near, far, n_samples, rand=False):
    ...
    # points = ...
    points = rays_o[..., None, :] + t[..., :, None] * rays_d[..., None, :]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    one_e_10 = 1e10
    # dists = ...
    dists = torch.cat([t[..., 1:] - t[..., :-1], torch.tensor([one_e_10], device=device).expand(t[..., :1].shape)], dim=-1)

    # alpha = ...
    alpha = 1.0 - torch.exp(-sigma * dists)
    # weights = ...
    T = cumprod_exclusive(torch.exp(-sigma * dists))
    weights = T * alpha
    ...
    # Compute the opacity (alpha) for each point
