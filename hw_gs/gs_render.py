...
def build_covariance_2d(mean3d, cov3d, viewmatrix, fov_x, fov_y, focal_x, focal_y):
    ...
    # cov2d = ...
    cov3d_view = W @ cov3d @ W.transpose(-1, -2)
    cov2d = J[..., :2, :] @ cov3d_view @ J[..., :2, :].transpose(-1, -2)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                tile_left = w
                tile_right = w + TILE_SIZE
                tile_top = h
                tile_bottom = h + TILE_SIZE

                rect_min, rect_max = rect

                gaussian_left = rect_min[:, 0]
                gaussian_right = rect_max[:, 0]
                gaussian_top = rect_min[:, 1]
                gaussian_bottom = rect_max[:, 1]

                # in_mask = .....
                in_mask = (
                    (gaussian_left < tile_right) & 
                    (gaussian_right > tile_left) & 
                    (gaussian_top < tile_bottom) & 
                    (gaussian_bottom > tile_top)
                ) # B 2
                #############################################################################
                #                             END OF YOUR CODE                              #
                #############################################################################
                ...
                sorted_depths, index = torch.sort(depths[in_mask]) # B 1 -> P 1
                ...
                # Step 1
                dx_reshape = dx.unsqueeze(-1)  # [B P 2 1]
                
                dx_transpose = dx_reshape.transpose(-1, -2) # [B P 1 2]
                # inverse: P 2 2
                # print("dx_transpose", dx_transpose.shape)
                # print("dx_reshape", dx_reshape.shape)
                # print("dx", dx.shape)
                # print("inverse_conv", sorted_inverse_conv.shape)
                
                gauss_weight = torch.exp(-0.5 * (dx_transpose @ sorted_inverse_conv @ dx_reshape).squeeze(-1, -2)) # [B P]

                # Step 2 
                alpha = (gauss_weight[..., None] * sorted_opacity[None]).clip(max=0.99) # [B P 1] * [1 P 1] = [B P 1]
                ...
                tile_color = (weights * sorted_color[None]).sum(dim=1)  # B P 1 * 1 P 3 = B 3
                background_color = torch.ones_like(tile_color)
                tile_color = tile_color + (1 - acc_alpha) * background_color # B 3 + B 1 = B 3

                tile_depth = (weights * sorted_depths[None, :, None]).sum(dim=1)  # B P 1 * 1 P 1, B 1
                #############################################################################
                #                             END OF YOUR CODE                              #
                #############################################################################
